    ...
    public class QuotesController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll([FromQuery] int? clientId = null)
        {
            ...
            var dt = await _db.QueryAsync(
                $"SELECT * FROM Quotes {where} ORDER BY IssueDate DESC", p);
        ...
        public async Task<IActionResult> Create([FromBody] CreateQuoteRequest req)
        {
            ...
            bool ok = await _db.ExecuteAsync(
                "INSERT INTO Quotes (ClientId, IssueDate, ExpirationDate, Total) " +
                "VALUES (@cId, GETDATE(), @exp, @total)",
        ...
        {
            bool ok = await _db.ExecuteAsync(
                "UPDATE Quotes SET ClientId=@cId, ExpirationDate=@exp, Total=@total WHERE Id=@id",
                new Dictionary<string, object>
                {
                    ["@id"] = id,
                    ["@cId"] = req.ClientId,
